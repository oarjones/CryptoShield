    ...
    : config_(config) // Ahora recibe y guarda la configuración correcta
{
    ...
    bool FileExtensionMonitor::MatchesSuspiciousPattern(const std::wstring& extension) const
    {
        // Usamos la lista de patrones regex de la configuración.
        const auto& patterns = config_.suspicious_patterns_regex;

        for (const auto& pattern_str : patterns) {
            try {
                // Creamos un objeto wregex para cada patrón.
                std::wregex pattern_regex(pattern_str, std::regex_constants::icase);

                // Comprobamos si la extensión coincide con el patrón.
                if (std::regex_match(extension, pattern_regex)) {
                    return true;
                }
            }
            catch (const std::regex_error& e) {
                // Opcional: Loggear si un patrón regex en el config es inválido.
